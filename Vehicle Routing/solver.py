...
def solve_it(input_data):
    ...
    # проверяет правильность количества обслуживаемых клиентов
...
def find_neigh(curr_sol, customers, vehicle_capacity):
    ...
    r1 = random.randint(1, len(customers)-1) # клиент
    ...
    r2 = s[random.randint(0, len(s)-1)][0] # транспортное средство
    r3 = random.randint(0, len(curr_sol[r2])) # позиция
    neigh[r2].insert(r3, customers[r1])
    return neigh

def find_neigh_2(curr_sol, customers, vehicle_capacity):
    neigh = deepcopy(curr_sol)
    v1 = random.randint(0, len(curr_sol)-1) # транспортное средство
    while len(curr_sol[v1]) == 0:
        v1 = random.randint(0, len(curr_sol)-1)
    c1 = random.randint(0, len(curr_sol[v1])-1) # клиент
    tmp = neigh[v1][c1]
    cap1, cap2 = vehicle_capacity+1, 0
    while cap1 > vehicle_capacity or cap2 < tmp.demand:
        v2 = random.randint(0, len(curr_sol)-1) # транспортное средство
        c2 = random.randint(0, len(curr_sol[v2])) # позиция
